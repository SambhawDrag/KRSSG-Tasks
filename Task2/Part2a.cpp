...
public:
	...
	void transitionup(int* curpos, int enter, char tr, int out, int num)
	{
		int* p = curpos;
		int i;
		cout<<"Lift "<<num<<"at floor: "<<*curpos<<" State: REST"<<endl;
		if (*curpos <= enter)
		{
		...
		*(curpos) = out;
		*p = out;
		cout<<"Lift "<<num<<"at floor: "<<out<<" State: REST"<<endl;
...
int main(int argc,char *argv[])
{
	...
	sys.lift1.current_pos=sys.lift1.initial_pos;
	sys.lift2.current_pos=sys.lift2.initial_pos;
	...
	enter=1; out=1;trans='R';
	input(&newSocket);  //Taking The first input
	sys.lift1.state = trans; sys.lift2.state= trans;
	int i1=0, i2=0;
	int num;
    while (!(enter==0 && trans=='0' && out==0))
	{
		...
		switch(num)
		{
			...
			case 2:   //FSM-2
				cout<<"Assigned to Lift2: "<<endl;
				sys.lift2.in[i2]=enter;
				sys.lift2.tr[i2]=trans;
				sys.lift2.out[i2]=out;
				switch (sys.lift2.state)
				{
				case 'U':
					sys.transitionup(&sys.lift2.current_pos, sys.lift2.in[i2], sys.lift2.tr[i2], sys.lift2.out[i2],num);
					sys.lift2.state = 'R';
					i2++;
					break;

				case 'D':
					sys.transitiondown(&sys.lift2.current_pos, sys.lift2.in[i2], sys.lift2.tr[i2], sys.lift2.out[i2], num);
					sys.lift2.state = 'R';
					i2++;
					break;
				...
				case 'R':
					...
					switch (sys.lift2.tr[i2])
					{
					case 'U': sys.lift2.state = 'U';
						break;
					case 'D': sys.lift2.state = 'D';
						break;
					case 'R': sys.lift2.state = 'R';
						i2++;
						break;
					}
					break;
